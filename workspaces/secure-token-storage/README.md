...
- user-approved caller-bound grants, revocation, and service-authenticated
  access token retrieval at `/api/secure-token-storage/token`.
...
`/connections/:provider/callback`, and `/connections/:sessionId/consent`. The
unauthenticated health check is `/api/secure-token-storage/health`. The local
configuration uses an in-memory SQLite database and must not be reused as a
production configuration. The local host does not register a provider adapter
unless these provider settings are supplied.
